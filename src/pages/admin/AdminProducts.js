...
const AdminProducts = () => {
  ...
  const [filters, setFilters] = useState({ sortBy: 'popular', category: '', gender: '' });
        ...
        <div className="filter-group">
          ...
          {['Popularity', 'Price (low to high)', 'Price (high to low)', 'Discount'].map(opt => (
            <label key={opt} className="filter-option">
              <input type="radio" name="sort" /> {opt}
            </label>
          ))}
        </div>
        <div className="filter-group">
      ...
      <div className="products-main">
        ...
        {editProduct && (
          <div style={{
            ...
            <div style={{ background: '#1a1a2e', padding: '30px', borderRadius: '8px', width: '500px', maxHeight: '80vh', overflowY: 'auto' }}>
                ...
                {['name', 'description', 'price', 'originalPrice', 'thumbnailImg'].map(field => (
                  <div className="form-group" key={field} style={{ marginBottom: '10px' }}>
                    ...
                    <input
                      ...
                      style={{ background: '#2a2a3e', border: '1px solid #444', color: 'white', width: '100%', padding: '8px', borderRadius: '4px' }}
                    />
                  </div>
                ))}
                <div style={{ display: 'flex', gap: '10px', marginTop: '15px' }}>
                  <button type="submit" className="btn-blue">Update</button>
                  <button type="button" className="btn-red" onClick={() => handleDelete(editProduct._id)}>Delete</button>
                  <button type="button" onClick={() => setEditProduct(null)} style={{ background: '#555', color: 'white', border: 'none', padding: '7px 14px', borderRadius: '4px', cursor: 'pointer' }}>Cancel</button>
                </div>
              </form>
            </div>
          </div>
